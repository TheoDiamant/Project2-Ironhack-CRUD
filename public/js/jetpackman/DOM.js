...
function mainMenu() {
  ...
  ctx.fillText(
    ...
    myGameArea.canvas.width / 2 - 40,
    myGameArea.canvas.height / 2 - 70
  );
  ctx.fillText(
    "Arrow Left -> Move Left ",
    myGameArea.canvas.width / 2 - 40,
    myGameArea.canvas.height / 2 - 20
  );
  ctx.fillText(
  ...
  ctx.fillText(
    ...
    myGameArea.canvas.width / 2 - 40,
    myGameArea.canvas.height / 2 + 140
  );
...
function start() {
  ...
  background.img.src = "/images/jetpackman-images/game_background_1.png";

  // Generate Pipes & Gallons
  for (let i = 0; i < 10; i++) {
    pipesDown.push(new Pipe(Pipe.distance * i, i * Pipe.height));
    myGameArea.gallons.push(
      new Gallon(Pipe.distance * i + Gallon.distance, i * Pipe.height + 500)
    );
  }
...
function upgradeHealth() {
  ...
  if (player.coins >= player.hcost) {
    ...
    document.getElementById(
      "upCostHealth"
    ).innerText = `Upgrade ${player.hcost}$`;
    document.getElementById("coins").innerText = "Coins: " + player.coins + "$";
    let obj = {
      name: player.playerName,
      coins: player.coins,
...
function pauseGame() {
  ...
  ctx.fillStyle = "rgb(48, 46, 46,0.5)";
  ctx.fillRect(
    myGameArea.canvas.width / 2 - 250,
    myGameArea.canvas.height / 2 - 250,
    500,
    500
  );
  ctx.font = "80px VT323";
  ...
  ctx.fillText(
    "Jet Pack Man!",
    myGameArea.canvas.width / 2,
    myGameArea.canvas.height / 2 - 160
  );
  ctx.font = "30px VT323";
  ctx.fillText(
    "Made by Gonçalo Vilaboim",
    myGameArea.canvas.width / 2,
    myGameArea.canvas.height / 2 - 120
  );
  ctx.fillStyle = "white";
  ctx.font = "italic 13pt Courier";
  ctx.fillText(
  ...
  setInterval(() => {
    if (a && isPaused) {
      ...
      ctx.fillText(
        "Game Paused!",
        myGameArea.canvas.width / 2,
        myGameArea.canvas.height / 2 + 220
      );
      a = false;
    } else if (!a && isPaused) {
      ctx.fillStyle = "white";
  ...
  switch (key) {
    ...
    case "Enter":
      ...
      ctx.fillText(
        ...
        myGameArea.canvas.width / 2,
        myGameArea.canvas.height / 2 + 175
      );
    ...
    case "s":
      if (myGameArea.isGameOver) {
        ...
        document.body.style.backgroundImage = `url('/images/jetpackman/game_background_1.png')`;
        ...
        document.getElementById("upCostGun").innerText = `Upgrade ${
          25 * obj.gun
        }$`;
        document.getElementById("upCostHealth").innerText = `Upgrade ${
          25 * obj.healt
        }$`;
        document.getElementById("upCostJet").innerText = `Upgrade ${
          25 * obj.jetpack
        }$`;
